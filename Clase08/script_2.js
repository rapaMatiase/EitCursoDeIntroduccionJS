...
const PANEL_DE_JUEGO = document.getElementById("tableroDeTateti")
...
function CargarJuego() {
    ...
    for (let i = 0; i < tableroTateti.length; i++) {
        for (let j = 0; j < tableroTateti[i].length; j++) {
            //Creo el elemento section para representar un casilla del tateti
            const elSection = document.createElement("section")
            //Le seteo la posicion en la matrix.
            elSection.setAttribute("data-x", i)
            elSection.setAttribute("data-y", j)
            //Agrego el evento
            elSection.addEventListener("click", function (event) {
                let elTarget = event.target

                let x = elTarget.getAttribute("data-x")
                let y = elTarget.getAttribute("data-y")

                play(x, y, elTarget)
            })

            PANEL_DE_JUEGO.appendChild(elSection)
        }
    }
}
...
EL_BUTTON_REPLAY.addEventListener("click", function () {
    ...
    let elSections = document.querySelectorAll("#tableroDeTateti section")
